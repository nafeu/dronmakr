#!/usr/bin/env bash
# Shared Linux E2E runner (run inside VM — no SSH orchestration).
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"

LABEL="${E2E_LINUX_LABEL:-linux}"
LOG_PREFIX="e2e-${LABEL}"

export DRONMAKR_TEST=1
export DRONMAKR_ASYNC_MODE="${DRONMAKR_ASYNC_MODE:-threading}"
export DRONMAKR_TEST_ROOT="${DRONMAKR_TEST_ROOT:-$(mktemp -d "/tmp/dronmakr-${LABEL}-e2e-XXXXXX")}"
...
if [[ ! -d venv ]]; then
  echo "${LOG_PREFIX}: venv/ missing — run one-time dev setup (see docs/e2e-vm-setup.md)" >&2
  exit 1
fi
...
echo "${LOG_PREFIX}: checking Surge XT prerequisite..."
require_surge_xt
...
if [[ -z "${DISPLAY:-}" ]]; then
  if command -v Xvfb >/dev/null 2>&1; then
    ...
    export DISPLAY=:99
    sleep 1
    echo "${LOG_PREFIX}: started Xvfb on ${DISPLAY}"
  else
    echo "${LOG_PREFIX}: DISPLAY unset and Xvfb missing — install xvfb" >&2
    exit 1
  fi
fi
...
if [[ "${E2E_BACKEND_ONLY:-0}" == "1" ]]; then
  echo "${LOG_PREFIX}: backend-only mode"
  ...
  echo "${LOG_PREFIX}: launching packaged app ${APP_BIN}"
...
echo "${LOG_PREFIX}: running Playwright..."
PLAYWRIGHT_HTML_REPORT="${E2E_ARTIFACTS_DIR}/report" \
  npx playwright test --config=playwright.config.ts "$@"

echo "${LOG_PREFIX}: PASS (artifacts: ${E2E_ARTIFACTS_DIR})"
